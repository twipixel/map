...
import prefix from '../../../src/console/prefix';
...
export default class App {
  ...
  testPbf(url, timeLabel) {
    console.time(prefix(`${timeLabel} 로드`));
    request({
      url,
      responseType: 'arraybuffer',
    }).then(buffer => {
      console.timeEnd(prefix(`${timeLabel} 로드`));
      console.time(prefix(`${timeLabel} pbf 파싱`));
      const tile = new VectorTile(new ProtoBuf(buffer));
      console.timeEnd(prefix(`${timeLabel} pbf 파싱`));
      // console.log(stringify(tile, 2));
      console.log(tile);
      console.time(prefix(`${timeLabel} 피쳐 생성`));
      const collection = this.getFeatures(tile);
      console.timeEnd(prefix(`${timeLabel} 피쳐 생성`));
  ...
  testOzone(url, timeLabel) {
    ...
    request({
      ...
      console.timeEnd(prefix(`${timeLabel} 로드`));
      console.time(prefix(`${timeLabel} pbf 파싱 및 피쳐 생성`));
      ...
      console.timeEnd(prefix(`${timeLabel} pbf 파싱 및 피쳐 생성`));
